...
if ($terupload) {
    $query = mysqli_query($koneksi,"INSERT INTO `donasi` (`id`,`email_user`, `nominal`, `metode_pembayar`, `bukti_transfer`, `waktu`) VALUES (null,'$resEmail[0]', '$tmp[1]', 'ovo', '$final_name', '$date')");
    if ($query){
        $var = $_SERVER['DOCUMENT_ROOT'].'/tape/';
        $status = array('message' => 'sukses');
        $encode = json_encode($status,true);
        $mess = json_decode($encode)->message;
        header("location: http://localhost/tape/donasi-pemberitahuan.php/?function=getDataProfil&id=$getid&nominal=&payment-method=ovo");
    }else {
